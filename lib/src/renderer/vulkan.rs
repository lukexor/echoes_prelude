...
use vulkanalia::{
    ...
    vk::{
        self, DeviceV1_0, EntryV1_0, ExtDebugUtilsExtension, Handle, HasBuilder, InstanceV1_0,
...
impl Context {
            ...
            unsafe fn check_physical_device(
                ...
                unsafe fn check_physical_device_extensions(
                    ...
                    DEVICE_EXTENSIONS
                        ...
                        .then_some(())
                        .ok_or_else(|| anyhow!("Missing required device extensions."))
                }
                ...
                if support.formats.is_empty() || support.present_modes.is_empty() {
                    bail!("Insufficient swapchain support.");
                }

                let features = instance.get_physical_device_features(physical_device);
                if features.sampler_anisotropy != vk::TRUE {
                    bail!("No sampler anisotropy.");
                }
    ...
    unsafe fn begin_single_time_cmd(device: &Device, data: &Data) -> Result<vk::CommandBuffer> {
        // Allocate
    ...
    unsafe fn transition_image_layout(
        ...
        device.cmd_pipeline_barrier(
            ...
            &[vk::MemoryBarrier::default(); 0],
            &[vk::BufferMemoryBarrier::default(); 0],
            &[barrier],
        );
        ...
        for model in &models {
            for index in &model.mesh.indices {
                ...
                for pos in &vertex.pos {
                    if !pos.is_finite() {
                        bail!("vertex position contains NaN or Infinity");
                    }
                }
                for texcoord in &vertex.texcoords {
                    if !texcoord.is_finite() {
                        bail!("vertex texcoords contains NaN or Infinity");
                    }
                }
    ...
    unsafe fn create_swapchain_image_views(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn create_descriptor_set_layout(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn create_pipeline(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn create_framebuffers(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn generate_mipmaps(
        ...
        {
            bail!("Texture image format does not support linear blitting!");
        }
    ...
    unsafe fn create_texture_image_view(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn create_texture_sampler(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn get_depth_format(instance: &Instance, data: &Data) -> Result<vk::Format> {
        let candidates = &[
    ...
    unsafe fn create_sync_objects(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn create_descriptor_pool(device: &Device, data: &mut Data) -> Result<()> {
    ...
    unsafe fn create_descriptor_sets(device: &Device, data: &mut Data) -> Result<()> {
        //  Allocate
        ...
        for i in 0..count {
            ...
            device.update_descriptor_sets(
                ...
                &[vk::CopyDescriptorSet::default(); 0],
            );
        }
        Ok(())
    }

    unsafe fn create_command_buffers(device: &Device, data: &mut Data) -> Result<()> {
